...
func TestPgSQLArchive_FetchArchiveMessages(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"by bare jid": {
			filters:     &archivemodel.Filters{With: "noelia@jackal.im", MaxCount: 20},
			withArgs:    []driver.Value{"ortuman", "noelia@jackal.im"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND \(to_bare = \$2 OR from_bare = \$2\)\) ORDER BY created_at LIMIT 20`,
		},
		"by full jid": {
			filters:     &archivemodel.Filters{With: "noelia@jackal.im/yard", MaxCount: 20},
			withArgs:    []driver.Value{"ortuman", "noelia@jackal.im/yard"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND \("to" = \$2 OR "from" = \$2\)\) ORDER BY created_at LIMIT 20`,
		},
		"by ids": {
			filters:     &archivemodel.Filters{Ids: []string{"id1234", "id5678"}, MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", "id1234", "id5678"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND id IN \(\$2,\$3\)\) ORDER BY created_at LIMIT 10`,
		},
		"by before id": {
			filters:     &archivemodel.Filters{BeforeId: "id1234", MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", "id1234", "ortuman"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND \(serial < \(SELECT serial FROM archives WHERE "id" = \$2 AND archive_id = \$3\)\)\) ORDER BY created_at LIMIT 10`,
		},
		"by after id": {
			filters:     &archivemodel.Filters{AfterId: "id1234", MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", "id1234", "ortuman"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND \(serial > \(SELECT serial FROM archives WHERE "id" = \$2 AND archive_id = \$3\)\)\) ORDER BY created_at LIMIT 10`,
		},
		"by before and after id": {
			filters:     &archivemodel.Filters{BeforeId: "id1234", AfterId: "id5678", MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", "id1234", "ortuman", "id5678", "ortuman"},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND \(serial < \(SELECT serial FROM archives WHERE "id" = \$2 AND archive_id = \$3\)\) AND \(serial > \(SELECT serial FROM archives WHERE "id" = \$2 AND archive_id = \$3\)\)\) ORDER BY created_at LIMIT 10`,
		},
		"by start timestamp": {
			filters:     &archivemodel.Filters{Start: timestamppb.New(starTm), MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", starTm},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND created_at > \$2\) ORDER BY created_at LIMIT 10`,
		},
		"by end timestamp": {
			filters:     &archivemodel.Filters{End: timestamppb.New(endTm), MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", endTm},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND created_at < \$2\) ORDER BY created_at LIMIT 10`,
		},
		"by start and end timestamp": {
			filters:     &archivemodel.Filters{Start: timestamppb.New(starTm), End: timestamppb.New(endTm), MaxCount: 10},
			withArgs:    []driver.Value{"ortuman", starTm, endTm},
			expectQuery: `SELECT id, "from", "to", message, created_at FROM archives WHERE \(archive_id = \$1 AND created_at > \$2 AND created_at < \$3\) ORDER BY created_at LIMIT 10`,
		},
	}
	for tn, tc := range tcs {
